...
#[test]
fn mint_with_token_id_should_work() {
    ExtBuilder::default().build().execute_with(|| {
        let next_class_id = NonFungibleTokenModule::next_class_id();
        assert_eq!(next_class_id, CLASS_ID);
        assert_ok!(NonFungibleTokenModule::create_class(&ALICE, vec![1], ()));
        assert_eq!(NonFungibleTokenModule::next_token_id(CLASS_ID), 0);
        assert_ok!(NonFungibleTokenModule::mint_with_token_id(&BOB, CLASS_ID, 5 ,vec![1], ()));
        assert_ok!(NonFungibleTokenModule::mint_with_token_id(&BOB, CLASS_ID, 6 ,vec![1], ()));
        // Ensure next token id not increasing
        assert_eq!(NonFungibleTokenModule::next_token_id(CLASS_ID), 0);
    });
}
